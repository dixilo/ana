...
from toast.rng import random as toast_rng

from toast.mpi import MPI, MPIShared

from toast._libtoast import (
...
class AtmSimMod(object):
    ...
    def get_volume(self):
        ...
        # z_max determination
        z_func = lambda el: -self._sinel0*np.cos(self._delta_az/2)*np.cos(el) + self._cosel0*np.sin(el)
        dz_func = lambda el: self._sinel0*np.cos(self._delta_az/2)*np.sin(el) + self._cosel0*np.cos(el)

        dz_min = dz_func(self._elmin)
        dz_max = dz_func(self._elmax)

        if dz_min*dz_max < 0:
            # Derivatives have different sign
            # => maximum between min and max
            tanel0 = self._sinel0/self._cosel0
            # Derivative becomes 0 at this El
            tanel = -1/(tanel0*np.cos(self._delta_az/2))
            cosel = 1/np.sqrt(1+tanel**2)
            sinel = tanel/np.sqrt(1+tanel**2)

            z_max = r*(-self._sinel0*np.cos(self._delta_az/2)*cosel + self._cosel0*sinel)
        else:
            z_max_min = z_func(self._elmin)
            z_max_max = z_func(self._elmax)

            z_max = r*max(z_max_min, z_max_max)
